...
def get_system_stats():
    ...
    stats = {
        "cpu": {
            ...
            "cores": {"value": psutil.cpu_count(logical=False), "unit": "physical cores"},
            "threads": {"value": psutil.cpu_count(logical=True), "unit": "logical cores"},
            "frequency": {"value": cpu_freq.current if cpu_freq else None, "unit": "MHz"}
        },
        "memory": {
            "total": {"value": round(memory.total / (1024**3), 2), "unit": "GB"},
            "used": {"value": round(memory.used / (1024**3), 2), "unit": "GB"},
            "available": {"value": round(memory.available / (1024**3), 2), "unit": "GB"},
        ...
        "network": {
            "bytes_sent": {"value": round(net_io.bytes_sent / (1024**2), 2), "unit": "MB"},
            "bytes_received": {"value": round(net_io.bytes_recv / (1024**2), 2), "unit": "MB"}
        },
        "system_info": {
            "os": platform.system(),
